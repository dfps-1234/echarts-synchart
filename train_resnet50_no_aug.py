#!/usr/bin/env python3
"""
train_resnet50_no_aug.py - 无数据增强的 ResNet50（消融实验）
"""
...
OUTPUT_MODEL = "chart_classifier_resnet50_no_aug.pth"
...
def main():
    ...
    train_dataset = ChartDataset(TRAIN_JSONL, IMAGE_DIR, label_to_id, transform=base_transform)
    val_dataset = ChartDataset(VAL_JSONL, IMAGE_DIR, label_to_id, transform=base_transform)
    test_dataset = ChartDataset(TEST_JSONL, IMAGE_DIR, label_to_id, transform=base_transform)
    ...
    best_val_acc = 0.0
    print("Starting training (no augmentation)...")
